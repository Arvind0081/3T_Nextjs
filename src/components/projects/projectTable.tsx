...
const ProjectTable = ({
  ...
  const handleSort = (key: string) => {
    ...
    return router.push(
      `${url}/?page=${data.pageNumber}&size=${data.pageSize}&status=${data.projectStatus}&search=${data.searchValue}&startDate=${data.startDate}&endDate=${data.endDate}&hiringStatus=${data.hiringStatus}&bilingType=${data.bilingType}&departmentId=${data.departmentId}&teamAdminId=${data.teamAdminId}&sortColumn=${key}&sortOrder=${direction}`
    );
  };
      ...
      <table className="table text-nowrap table-hover border table-bordered">
        ...
        <tbody>
          {allProjects.map((project: any) => (
            <tr key={project.id}>
              <td className="text-nowrap">{project.id}</td>
              <td>
                <Link
              ...
              <td>{format(new Date(project.createdTime), "yyyy-MM-dd")}</td>
              <td>
                {
                  billingType?.filter(
                    (item: { value: number }) => item.value == project.isBilling
                  )[0]?.text
                }
              </td>
              <td>
                {
                  hiringType?.filter(
                    (item: { value: number }) =>
                      item.value == project.hiringStatus
                  )[0]?.text
                }
              </td>
              <td>
