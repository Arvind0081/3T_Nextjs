...
} from '@/utils/finalAttendanceStatus';
...
const AttendanceReport = ({
  ...
  const showingRecordCount = () => {
    const totalCount = attendanceReportList?.totalCount || 0;
    const currentPage = param.pageNo || 1;
  ...
  const bioMetricAttendanceStatus = (
    bioMetricAttendance: any,
    item: any,
    index: number
  ) => {
    return bioMetricAttendance?.filter(
      (emp: any) => emp.employeeCode === item.employeeNumber
    )[index]?.statusCode === 'A'
      ? 'Ab'
                ...
                <tbody>
                  {attendance?.map((item: any) => (
                    <tr key={item.employeeNumber}>
                      <th>
                        <h3 className="userName">{item.employeeName}</h3>{' '}
                        <h3 className="EMP_no">{item.employeeNumber}</h3>{' '}
                      </th>
                              ...
                              <div className="attendance_status_info">
                                ...
                                >
                                  {finalAttendanceStatus(
                                    ...
                                    bioMetricAttendance?.filter(
                                      (emp: any) =>
                                        emp.employeeCode === item.employeeNumber
                                    )[index]
                                  )}
                                  {/* {
                                employeeDetail.attendanceStatus
                                ...
                                <div className="_BM_attendance">
                                  <div
                                    className={`attendance_status_3t ${attendanceStatusClass(employeeDetail.attendanceStatus)}`}
                                  >
                                  ...
                                  <div
                                    className={`attendance_status_bm ${attendanceStatusClass(bioMetricAttendanceStatus(bioMetricAttendance, item, index))}`}
                                    onClick={(e) => {
                                      bioMetricAttendance?.filter(
                                        (emp: any) =>
                                          emp.employeeCode ===
